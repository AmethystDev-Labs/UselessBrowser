...
class BehaviorSpoofModule:
    ...
    def human_click_with_movement(self, page, element, from_pos: tuple = None):
        ...
        try:
            ...
            # Small jitter around target before click
            for _ in range(random.randint(2, 4)):
                jx = end_x + random.randint(-4, 4)
                jy = end_y + random.randint(-4, 4)
                page.run_js(f'''
        const event = new MouseEvent('mousemove', {
            clientX: {jx},
            clientY: {jy},
    ...
    def _system_cursor_available(self) -> bool:
        return _USER32 is not None

    def _get_cursor_pos(self):
        if not self._system_cursor_available():
            return None
        pt = wintypes.POINT()
    ...
    def _send_mouse_input(self, x: int, y: int, flags: int):
        mi = _MOUSEINPUT(x, y, 0, flags, 0, _ULONG_PTR(0))
        inp = _INPUT(_INPUT_MOUSE, mi)
        _USER32.SendInput(1, ctypes.byref(inp), ctypes.sizeof(inp))

    def _move_cursor_abs(self, x: int, y: int):
        width, height = self._screen_size()
        if width <= 1 or height <= 1:
            return
        x = max(0, min(width - 1, int(x)))
        y = max(0, min(height - 1, int(y)))
        abs_x = int(x * 65535 / (width - 1))
        abs_y = int(y * 65535 / (height - 1))
        self._send_mouse_input(abs_x, abs_y, _MOUSEEVENTF_MOVE | _MOUSEEVENTF_ABSOLUTE)

    def _click_system(self):
        self._send_mouse_input(0, 0, _MOUSEEVENTF_LEFTDOWN)
        time.sleep(random.uniform(0.03, 0.08))
        self._send_mouse_input(0, 0, _MOUSEEVENTF_LEFTUP)

    def _get_element_screen_center(self, page, element):
        try:
            data = page.run_js('''
                const rect = arguments[0].getBoundingClientRect();
                return {
        ...
        offset_x = max(0, (data['outerWidth'] - data['innerWidth']) / 2)
        offset_y = max(0, data['outerHeight'] - data['innerHeight'] - offset_x)
        x = data['screenX'] + offset_x + data['x'] + data['width'] / 2
        y = data['screenY'] + offset_y + data['y'] + data['height'] / 2
        dpr = data.get('dpr', 1) or 1
        return (x * dpr, y * dpr)

    def human_click_with_system_cursor(self, page, element, from_pos: tuple = None):
        if not self._system_cursor_available():
            return self.human_click_with_movement(page, element, from_pos=from_pos)

        target = self._get_element_screen_center(page, element)
        if not target:
            return self.human_click_with_movement(page, element, from_pos=from_pos)

        start = from_pos or self._get_cursor_pos()
        if not start:
            start = (target[0] + random.randint(-40, 40), target[1] + random.randint(-40, 40))
        ...
        for i in range(steps + 1):
            ...
            x = int(bezier(t, start_x, ctrl1_x, ctrl2_x, end_x))
            y = int(bezier(t, start_y, ctrl1_y, ctrl2_y, end_y))
            self._move_cursor_abs(x, y)
            time.sleep(random.uniform(0.005, 0.02))

        for _ in range(random.randint(2, 4)):
            jx = end_x + random.randint(-4, 4)
            jy = end_y + random.randint(-4, 4)
            self._move_cursor_abs(jx, jy)
            time.sleep(random.uniform(0.01, 0.03))
