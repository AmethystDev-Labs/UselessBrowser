...
class ProfilesMixin:
    ...
    def _set_profile_details(self, profile_id: Optional[str], profile: Optional[SpoofProfile]) -> None:
        ...
        if profile:
            screen_label = f'{profile.screen_width} x {profile.screen_height}'
            ratio_label = f'{profile.pixel_ratio:.2f}'
            hardware_label = f'{profile.hardware_concurrency} / {profile.device_memory} GB'
            self.field_screen.setText(screen_label)
            self.field_pixel_ratio.setText(ratio_label)
            self.field_hardware.setText(hardware_label)
            self.field_webgl.setText(profile.webgl_renderer)
            self.field_geo.setText(
                f'{profile.latitude:.4f}, {profile.longitude:.4f} ({profile.accuracy:.0f}m)'
            )
        else:
